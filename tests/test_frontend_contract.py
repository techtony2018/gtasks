...
state.goalExecution = {
  ...
  summary: {
    ...
    action_queue: [
      ...
      { owner: "system", kind: "repair_artifact_publisher_identity", label: "Repair Artifact publisher identity", goal_slug: "goals/private-token", task_slug: "tasks/private-token", todo_slug: "todos/private-token", todo_updated_at: "todo-private-v1", agent_slug: "agents/tammy", summary: "Provision dashboard Artifact publisher credentials and verify the Agent identity matches POST /api/artifacts.", detail: "Artifact publication failed with artifact_identity_mismatch.", blocked_goal_count: 2, related_questions: [
    ...
    next_action: "Answer the Timmy question for Which family-care scope should Toddy use next?; repair Tammy Artifact publisher identity for 2 blocked Goals; assign Entrepreneurship to Tammy (recommended: lowest verified Codex Goal load); executing or delivered Agent work can continue.",
  },
  last_run: {
...
assert(goalExecutionSurfaceText.includes("Next action: Answer the Timmy question for Which family-care scope should Toddy use next?; repair Tammy Artifact publisher identity for 2 blocked Goals"), "Goal execution surface did not expose the exact system repair next action");
...
assert(goalExecutionSurfaceText.includes("System action required"), "Goal execution surface did not label system-owned repair actions");
assert(goalExecutionSurfaceText.includes("Repair Artifact publisher identity"), "Goal execution surface did not expose Artifact publisher repair action");
assert(!goalExecutionSurfaceText.includes("Private input required"), "Artifact publisher identity mismatch was still shown as Tony private input");
...
assert(inboxGoalActionsText.includes("repair Tammy Artifact publisher identity for 2 blocked Goals"), "Inbox Goal execution actions omitted system repair next-action copy");
assert(inboxGoalActionsText.includes("System action required"), "Inbox Goal execution actions omitted system-owned repair action label");
...
globalThis.fetch = async (url, options = {}) => {
  planSequence.push(String(url).includes("/answer") ? "answer" : "owner");
...
loadAgents = async () => { planSequence.push("loadAgents"); };
loadGoalExecution = async () => { planSequence.push("loadGoalExecution"); };
loadAgentWork = async () => { planSequence.push("loadAgentWork"); };
render = () => {};
showToast = () => {};
...
assert(planSequence[0] === "answer" && planSequence[1] === "owner", "recommended unblock plan refreshed before performing the second explicit write");
state.agents = [
  { slug: "agents/timmy", name: "Timmy", runtime: "codex", default_goal_slugs: [goalSlug] },
];
let sparseOwnerRequests = [];
globalThis.fetch = async (url, options = {}) => {
  sparseOwnerRequests.push({ url: String(url), method: options.method, body: JSON.parse(options.body || "{}") });
  if (String(url).includes("/answer")) {
    return { ok: true, json: async () => ({ verified: true, next_owner: "agents/timmy", todo: {
      slug: "todos/question",
      parent_task: taskSlug,
...
state.goalExecution.summary = {
  ...
  blocking_questions: state.goalExecution.last_run.summary.blocking_questions,
  missing_owners: [],
  action_queue: [
    { owner: "tony", kind: "answer_question", label: "Answer Agent question", goal_slug: goalSlug, task_slug: taskSlug, todo_slug: "todos/question", todo_updated_at: "todo-v1", agent_slug: "agents/timmy", summary: "Which family-care scope should Toddy use next?", detail: "Choose the scope and first bounded action.", answer_template: "Scope categories: accepted\nDesired outcomes: accepted\nConstraints: accepted\nFirst action: approved\nNotes: Keep the work bounded to the stated scope, outcomes, constraints, and first action." },
  ],
  next_action: "Answer the Timmy question for Which family-care scope should Toddy use next?; the assigned Agent can resume blocked Goal work.",
};
state.goalExecution.last_run.summary = state.goalExecution.summary;
const answerOnlyInboxActions = renderGoalExecutionInboxActions();
const answerOnlyButtons = walkElements(answerOnlyInboxActions, (element) =>
  String(element.className || "").includes("goal-execution-unblock-plan"));
assert(answerOnlyButtons.length === 1, "answer-only Goal execution actions did not expose one recommended answer button");
assert(flattenText(answerOnlyButtons[0]).includes("Submit recommended answer"), "answer-only recommended action button had unclear copy");
let answerOnlyRequests = [];
globalThis.fetch = async (url, options = {}) => {
  answerOnlyRequests.push({ url: String(url), method: options.method, body: JSON.parse(options.body || "{}") });
  return { ok: true, json: async () => ({ verified: true, next_owner: "agents/timmy", todo: {
    slug: "todos/question",
    parent_task: taskSlug,
    status: "done",
...
render = originalRender;
showToast = originalShowToast;
assert(answerOnlyRequests.length === 1, "answer-only recommended action did not perform exactly one explicit write");
assert(answerOnlyRequests[0].url === "/api/todos/todos%2Fquestion/answer", "answer-only recommended action used the wrong endpoint");
assert(answerOnlyRequests[0].body.answer.includes("Scope categories: accepted"), "answer-only recommended action did not use the concrete answer draft");
...
"""
    ...
    def test_about_modal_is_beneath_gbrain_status_with_release_history(self) -> None:
        ...
        self.assertIn('id="sidebar-gbrain-version"', html)
        self.assertIn('id="about-dialog"', html)
        self.assertIn('role="dialog"', html)
        self.assertIn('aria-modal="true"', html)
        self.assertIn('id="about-close"', html)
        self.assertIn('id="release-history"', html)
        self.assertIn('id="about-gbrain-version"', html)
        self.assertIn('"/api/releases"', javascript)
        self.assertIn('"/api/health"', javascript)
        self.assertIn("gbrain_version", javascript)
...
"""
    ...
    def test_sidebar_icons_match_primary_control_size_and_version_moves_to_artwork(self) -> None:
        ...
        self.assertNotIn('id="sidebar-gbrain-version"', sidebar)
        self.assertIn('id="sidebar-version"', art_footer)
        self.assertIn('id="sidebar-gbrain-version"', art_footer)
        ...
        self.assertLess(art_footer.index("sidebar-version"), art_footer.index("sidebar-gbrain-version"))
...
"""
    ...
    def test_footer_controls_group_around_centered_word_art_without_duplicates(self) -> None:
        ...
        self.assertLess(footer.index('id="sidebar-version"'), footer.index('id="sidebar-gbrain-version"'))
        self.assertLess(footer.index('id="sidebar-gbrain-version"'), footer.index('id="logs-button"'))
        ...
        self.assertIn("flex-wrap: wrap", mobile_footer)
        self.assertIn("position: static", mobile_footer)
        self.assertIn("max-width: 100%", mobile_footer)
